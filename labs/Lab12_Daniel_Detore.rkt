...
(define (degree G v)
  (if (empty? G)
    0
    (cond
      ((and (= v (caar G)) (= v (cadar G)))
        (+ 2 (degree (cdr G) v)))
      ((or (= v (caar G)) (= v (cadar G))) 
        (+ 1 (degree (cdr G) v)))
      (else 
        (degree (cdr G) v))
    )
...
(define (all-even? G)
  (empty? (filter odd? (map (lambda (x) (degree G x)) (get-vertices G))))
)
...
(define (adjacent G v)
  (if (empty? G)
    ...
    (cond
      ((equal? v (caar G)) 
        (cons (cadar G) (usort (adjacent (cdr G) v))))
      ((equal? v (cadar G)) 
        (cons (caar G) (usort (adjacent (cdr G) v))))
...
(define (connected? G)
  (if (empty? G)
  #t
  (not (empty? (filter (lambda (x) (equal? (get-vertices G) x)) 
    (map usort (map (lambda (x) (dfs G `() x)) (get-vertices G)))
  )))
  )
)
