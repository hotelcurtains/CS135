...
(define (p- a b)
  (cond 
  [(and (equal? a z) (not (equal? b z))) (eopl:error "subtraction error: a < b!")] 
  [(equal? b z) a]
...
(define (p% a b)
  (cond
  [(p> b a) a]
  [(p= b z) (eopl:error "modulo error: mod is zero!")]
  [else (p% (p- a b) b)]
  ) 
)
...
(define (p-prime? p)
  (cond 
    ...
    [(p= p (succ (succ z))) #t]
    [(p-even? p) #f]
    [else (check-prime p (succ (succ (succ z))))]
  )
)

(define (check-prime p i)
  (cond
    [(p>= (p* (succ (succ z)) i) p) #t]
    [(pz? (p% p i)) #f]
    [else (check-prime p (p+ i (succ (succ z))))]
  )
)
