...
(define (encrypt plaintext key)
  (map (lambda (x) (mod-exp x (cadr key) (car key))) (map char->integer (string->list plaintext)))
)
...
(define (decrypt ciphertext key)
  (list->string (map integer->char (map (lambda (x) (mod-exp x (cadr key) (car key))) ciphertext)))
)
